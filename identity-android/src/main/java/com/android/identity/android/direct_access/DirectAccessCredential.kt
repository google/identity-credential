 ...
 *
 * Creation of a [DirectAccessCredential] is very similar to any other [Credential] with the following
 * exceptions:
 *
 * First, a slot must be reserved in the Direct Access applet for the [Document] this credential is
 * associated with. This can be done using [DirectAccess.allocateDocumentSlot]. Once allocated, the
 * the slot must be passed into the [DirectAccessCredential] constructor when creating new credentials.
 *
 ...
 *
 * Lastly, in order to use the credential, it must be set at the active credential in the Direct
 * Access applet using [setAsActiveCredential] once the credential is certified.
 */
...
class DirectAccessCredential: Credential {
     ...
     * [CredentialFactory.createCredential] when loading a [Document] instance from disk. Since
     * the credential is serialized by the [Document] and stored in [DocumentStore] in its serialized
     * CBOR form, creation of a [DirectAccessCredential] via deserialization does not require
